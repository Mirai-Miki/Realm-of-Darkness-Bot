...
module.exports = class DamageTracker20th
{
  ...
  updateAgg(amount)
  {
    ...
    let total = this.getTotalDamage()
    if (total > this.total) this.aggravated = this.aggravated - (total - this.total);
  }
  ...
  getTracker()
  {
    ...
    if (this.overflow)
    {
        tracker += `Damage overflowed by ${this.overflow} while ` +
